        ...
        .plot = function(image, ggtheme, theme, ...) {
            ...
            if (isCategorical) {
                ...
                p <- p + ggplot2::scale_fill_manual(
                    values = pal, na.value = 'white', name = valueTitle, na.translate = FALSE
                )
            } else {
                cmapName <- options$colourScheme
                if (cmapName %in% c('viridis', 'magma', 'plasma', 'cividis')) {
                    p <- p + ggplot2::scale_fill_viridis_c(
                        option = cmapName, na.value = 'white', name = valueTitle
                    )
                } else {
                    brewerPalette <- if (cmapName == 'reds') 'Reds' else 'Blues'
                    p <- p + ggplot2::scale_fill_distiller(
                        palette = brewerPalette, direction = 1, na.value = 'white', name = valueTitle
                    )
                }
            }
